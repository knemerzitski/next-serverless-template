 ...
 */
function transpileTypeScript(inputPath: string, options: PreTranspileOptions = {}): string {
...
export function transpileTypeScriptAsFile(
  ...
  const outPath = path.join(__dirname, 'out', path.format({ name: jsPathParsed.name, ext: '.js' }));
